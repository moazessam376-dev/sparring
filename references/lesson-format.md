...
4. **The mechanism, generic.** One diagram, built with the diagram-design skill (see Diagrams below) and embedded as inline SVG. Every diagram box has a caption saying what it is and why it is on the path. Define every term inline on first use with `<details>` or a styled `<abbr>` with a visible expansion.
