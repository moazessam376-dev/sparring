...
4. Write cards to a temporary JSON array without `id`, `added`, or `sched`, then run `drill.mjs add <project> <file.json>`. Each card names a `concept`, gives an answer-free `ask`, lists a one-to-six item `rubric`, includes `grounding`, `contexts`, and `source`. Set `contexts` to the bank project's own name plus at least one transfer world: `library`, `hospital`, `isp-support`, `ecommerce`, `school`, `bank`, `logistics`, or `generic`. Let the script assign ids and validate the input.
