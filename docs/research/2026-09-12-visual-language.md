...
Native effects are available, but they are not one portable Tauri API. On macOS, AppKit's `NSVisualEffectView` provides translucency and vibrancy, with `NSVisualEffectMaterial.sidebar` and a choice between `.behindWindow` and `.withinWindow` blending [S43, S44, S45, P]. Tauri's `window-vibrancy` crate exposes `apply_vibrancy` for macOS materials and, on macOS 26 or newer, `apply_liquid_glass`; the latter requires the macOS private API feature, a transparent window and a transparent HTML body, and should be treated as an optional enhancement because private API use can affect App Store acceptance [S46, P].
