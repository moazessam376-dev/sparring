...
test('open creates every table and records the schema version', () => {
  ...
  for (const expected of ['attempts', 'card_elo', 'card_sched', 'card_topics', 'cards', 'events', 'meta', 'projects', 'topic_elo', 'topic_prereqs', 'topic_projects', 'topics']) {
...
create table if not exists topics (
  id text primary key, name text not null, parent text, kind text not null
);
...
create table if not exists cards (
  ...
  rubric text not null, altitude text not null, grounding text not null, contexts text not null,
  source text not null, added text not null, retired integer not null default 0
);
create table if not exists card_topics (
  card text not null, topic text not null, primary key (card, topic)
);
create table if not exists attempts (
  id text primary key, card text not null, at text not null, grade text not null,
  question text, context text, answer text, gap text, mode text not null
);
create table if not exists card_sched (
  card text primary key, stability real, difficulty real, due text,
...
const HANDLERS = {
  ...
  'attempt.recorded': (db, e) => {
    db.prepare(`insert or replace into attempts (id, card, at, grade, question, context, answer, gap, mode)
      values (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(e.id, e.data.card, e.at, e.data.grade, e.data.question ?? null, e.data.context ?? null,
        e.data.answer ?? null, e.data.gap ?? null, e.data.mode ?? 'drill');
  },
  'grade.contested': (db, e) => {
    db.prepare('update attempts set grade = ? where id = ?').run(e.data.userGrade, e.data.attempt);
  },
  'lesson.completed': () => {
...
Run: `node --test core/ scripts/drill.test.mjs`
...
Add a section naming `core/` as the headless state layer, stating that the event log is the only durable record and the database is a rebuildable cache, and that `node --test core/` runs its tests.
