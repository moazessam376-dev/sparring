...
- Node 22.13 or newer. `node:sqlite` landed in 22.5 behind the `--experimental-sqlite` flag and only works without it from 22.13; the continuous integration run on 22.5.0 is what exposed this. The repository's current Node is 22.23.0.
...
**Interfaces:**
- Consumes: nothing. The implementation imports neither module; the test imports `open` from `core/db.mjs`.
- Produces: `apply(db, event): true | false | 'deferred'` returning false when the event was already applied, and `rebuild(db, events): number` returning how many were newly applied.
...
test('updating a card rewrites its grounding index, not just its json', () => {
  ...
  // node:sqlite returns null-prototype rows and deepEqual here is strict, so
  // project them before comparing.
  const rows = db.prepare('select path, commit_sha from card_grounding where card = ?').all('c001')
    .map((r) => ({ path: r.path, commit_sha: r.commit_sha }));
...
export function rebuild(db, events) {
  ...
  try {
    ...
    // is genuinely absent from the log rather than merely out of order, so it is
...
- Produces: `DEFAULT_W: number[]`, `gradeToRating(grade): 1|2|3`, `retrievability(elapsedDays, stability, w): number`, `nextState(state, grade, elapsedDays): {stability, difficulty}`, `nextInterval(stability, desiredRetention, w): number`, `schedule(state, grade, todayString): {stability, fsrs_difficulty, due, reps, lapses, last_grade, last_at}`, where `last_at` is what lets the next call know how many days elapsed.
...
The stability and difficulty update equations are not reproduced here and must be copied from the primary source while implementing. The equations live in `src/model_v6.rs` and `src/model.rs`, not in `src/inference_v6.rs`, which holds only the parameters and the decay. Fetch the raw files rather than the rendered pages, because the markdown conversion drops most of the Rust:

```
curl -s https://raw.githubusercontent.com/open-spaced-repetition/fsrs-rs/main/src/model_v6.rs
curl -s https://raw.githubusercontent.com/open-spaced-repetition/fsrs-rs/main/src/model.rs
curl -s https://raw.githubusercontent.com/open-spaced-repetition/fsrs-rs/main/src/simulation.rs
```

From `model_v6.rs`: the forgetting curve, the next interval, stability after success, stability after failure, short-term stability, initial difficulty, mean reversion and the difficulty update. From `model.rs`: initial stability, which is simply the weight at the rating's index, and the first-review path that uses it with a clamped initial difficulty and no mean reversion. From `simulation.rs`: the clamps, stability between 0.0001 and 36500 and difficulty between 1 and 10.

The algorithm wiki at `https://github.com/open-spaced-repetition/awesome-fsrs/wiki/The-Algorithm` is a useful cross-check on the curve, the interval, the difficulty delta, the mean reversion and the post-lapse stability. It is not sufficient on its own: its FSRS-6 section does not define initial stability, initial difficulty, or the success-branch stability.

Implement short-term stability as well. The source branches on elapsed days before it branches on rating, and a same-day repeat is normal in sparring. Without it a same-day answer computes a recall probability of 1, which makes the stability change exactly zero.

Write each equation with a comment naming the function it came from. Do not write any of them from memory.
...
Follow the source's sign convention: decay is the negation of the last weight, so `decay = -w[20]` and `factor = 0.9^(1/decay) - 1`. That is algebraically the same as writing both positive, and it lets each line be diffed against the Rust without a sign flip.

A check worth running by hand before moving on: the factor comes out at 0.980346, and `retrievability(S, S)` must be exactly 0.9000 for any stability. Try it at two different stabilities. If either misses, the factor derivation is wrong and not the test.
