...
slug: sparring
source-url: https://github.com/moazessam376-dev/sparring
created: 2026-09-10
updated: 2026-09-10
notes: Dark lesson theme for the sparring skill; use the dark variant only; amber accent; Inter for names, Geist Mono for technical labels
-->
# Style Guide
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#f5f5f5` (white-smoke) | `#0a0f0e` |
| `paper-2` | Diagram container bg, secondary fill | `#ececec` | `#0d1513` |
| `ink` | Primary text, primary stroke | `#2d3142` (jet-black) | `#ece9e1` |
| `muted` | Secondary text, default arrow stroke | `#4f5d75` (blue-slate) | `#a9b0ac` |
| `soft` | Sublabels, boundary labels | `#7a8399` | `#6f7a76` |
| `rule` | Hairline borders | `rgba(45,49,66,0.12)` | `#1d2a26` |
| `rule-solid` | Stronger borders, baselines | `#bfc0c0` (silver) | `rgba(169,176,172,0.35)` |
| `accent` | Focal / 1–2 max per diagram | `#eb6c36` (atomic-tangerine) | `#f2b544` (amber) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(235,108,54,0.08)` | `rgba(242,181,68,0.12)` |
| `link` | HTTP/API calls, external arrows | `#2e5aa8` | `#58d68d` |
...
|---|---|---|---|---|
| `title` | Inter | 1.75rem | 400 | Page H1 |
...
| `callout` | Inter *italic* | 14px | 400 | Editorial asides only |
...
**Load-bearing rule:** Mono is for *technical* content (ports, commands, URLs, field types). Names go in Geist sans. Page title is Inter. Italic Inter is reserved for annotation callouts (see [primitive-annotation.md](primitive-annotation.md)). **Never JetBrains Mono** as a blanket "dev" font.

### Traditional Chinese labels

Geist and Inter carry no Han. A Traditional Chinese `<text>` element extends its own family — never swap the skin:

```svg
<text font-family="'Geist', 'Noto Sans TC', 'PingFang TC', 'Microsoft JhengHei', sans-serif">請求項比對</text>
```

Both Noto TC faces ship in the font link above, so the web font resolves before any locally installed one and the same file renders identically on macOS, Windows, and a reviewer's browser. The local families follow it for offline viewing. Page titles need the serif equivalent — `'Inter', 'Noto Serif TC', serif` — or a mixed Latin/Han title resolves Han through the platform's generic serif and the two halves disagree. Google's `css2` endpoint slices Chinese by unicode-range, so a diagram with a handful of Chinese labels downloads only the slices it touches.
...
- **Serif + sans + mono**: three families, not more. If brand typography is all sans, keep Inter for `title` and `callout` anyway — the contrast is load-bearing.
